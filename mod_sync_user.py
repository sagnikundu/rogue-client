...
def adduser(user):
  ...
  f = open('devops_list','r'):
  userlist = f.readlines()
  f.close()

  for item in userlist:   
    devops_userlist.append(item.strip())

  if user in devops_userlist:
    os.system('/usr/sbin/adduser --shell /bin/bash --disabled-password --force-badname --ingroup devops --gecos ""  %s' % (user,))
  else:
